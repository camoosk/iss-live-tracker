(() => {
  ...
  function renderUser(location) {
    user = location;
    $('manual-lat').value = Number(location.latitude).toFixed(6);
    $('manual-lon').value = Number(location.longitude).toFixed(6);
    const sourceText = location.source === 'automatic' ? t('automaticLocation') : t('manualLocation');
    const accuracyText = Number.isFinite(Number(location.accuracy)) ? ` · ±${fmt(location.accuracy, 0)} m` : '';
    $('location-result').textContent = `${sourceText}: ${fmt(location.latitude, 5)}°, ${fmt(location.longitude, 5)}°${accuracyText}`;
  ...
  function renderDistance() {
    if (!user || !iss) return;
    const R = 6371;
    const lat1 = Number(user.latitude) * Math.PI / 180;
    const lat2 = Number(iss.latitude) * Math.PI / 180;
    const dLat = (Number(iss.latitude) - Number(user.latitude)) * Math.PI / 180;
    const dLon = (Number(iss.longitude) - Number(user.longitude)) * Math.PI / 180;
  ...
  $('locate-btn').addEventListener('click', async () => {
    ...
    try {
      await LocationController.locate();
    } catch (error) {
      console.warn('User geolocation unavailable:', error);
      $('location-result').textContent = t('locationDenied');
    } finally {
  ...
  $('manual-location-form').addEventListener('submit', event => {
    ...
    try {
      LocationController.setManual($('manual-lat').value, $('manual-lon').value);
    } catch (_) {
      $('location-result').textContent = t('invalidLocation');
    }
  });
